...
void displayTrains() {
    ...
    printf("Train Number\tsource\t\t\tDestination\t\tAvailable Seats\tReserved Seats\t \n");
    printf("##############################################################################################################\n");
    for (int i = 0; i < MAX_TRAINS; i++) {
        printf("%d\t%s\t%s\t\t%d\t\t\t%d\n", 167235+i,train[i].source, train[i].destination,train[i].availableSeats,train[i].reservedseats);
    }
}
...
//    train[trainNumber-167235].PassengerName="name";
