...
osThreadId sendTaskhandlinHandle;
...
int main(void)
{
  ...
  /* definition and creation of sendTaskhandlin */
  osThreadDef(sendTaskhandlin, StartTask03, osPriorityIdle, 0, 128);
  sendTaskhandlinHandle = osThreadCreate(osThread(sendTaskhandlin), NULL);
...
void MQTT_Init()
{
	...
	/*********CMD+MQTTCONCFG---->LED **********/
	//memset(&buffer[0],0x00,strlen(buffer));
	sprintf (&buffer[0], "CMD+MQTTCONCFG=3,mqtt-kumardoddi1996-j3oh68,,,,,,,,,\r\n");
	HAL_UART_Transmit(&huart1, (uint8_t*)buffer, strlen(buffer), 1000);
	HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), 1000);
	//memset(&buffer[0],0x00,strlen(buffer));
	//HAL_Delay(500);
...
/**
* @brief Function implementing the sendTaskhandlin thread.
* @param argument: Not used
* @retval None
*/
...
void send_temp(void *argument)
{
    for (;;)
    {
        ...
        char buffer[100];
        sprintf(buffer, "Temperature=%.2f°C, Humidity=%.2f%%\r\n", temperature, humidity); // @suppress("Float formatting support")
        HAL_UART_Transmit(&huart2, (uint8_t *)buffer, strlen(buffer), HAL_MAX_DELAY);

        // Send temperature and humidity to Rightech (MQTT)
        sprintf(buffer, "base/state/temperature=%.2f", temperature); // @suppress("Float formatting support")
        // Publish temperature to MQTT
        // You should implement your MQTT publish function here
        publish_to_mqtt(buffer);

        sprintf(buffer, "base/state/humidity=%.2f", humidity); // @suppress("Float formatting support")
